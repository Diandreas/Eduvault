...
<head>
    ...
    <style>
        .bg-gradient {
            background: linear-gradient(135deg, #FF2D20 0%, #FF8E53 100%);
        }
        .hover-scale {
    ...
    <header class="bg-white dark:bg-gray-800 shadow-md">
        <nav class="container mx-auto px-6 py-4">
            <div class="flex justify-between items-center">
                <h1 class="text-3xl font-bold text-[#FF2D20]">EduVault</h1>
                @if (Route::has('login'))
                    <div class="space-x-4">
                        @auth
                            <a href="{{ url('/dashboard') }}" class="font-semibold text-gray-600 hover:text-[#FF2D20] dark:text-gray-400 dark:hover:text-white transition duration-300">Tableau de bord</a>
                        @else
                            <a href="{{ route('login') }}" class="font-semibold text-gray-600 hover:text-[#FF2D20] dark:text-gray-400 dark:hover:text-white transition duration-300">Connexion</a>
                            @if (Route::has('register'))
                                <a href="{{ route('register') }}" class="font-semibold text-white bg-[#FF2D20] hover:bg-[#FF2D20]/80 px-4 py-2 rounded-lg transition duration-300">Inscription</a>
                            @endif
                        @endauth
                    </div>
                @endif
    ...
    <main class="flex-grow">
        <section class="bg-gradient text-white py-20">
            <div class="container mx-auto px-6 text-center">
                ...
                <a href="{{ route('register') }}" class="bg-white text-[#FF2D20] font-bold py-3 px-8 rounded-full text-lg hover:bg-gray-100 transition duration-300">
        ...
        <section class="py-20">
            <div class="container mx-auto px-6">
                <div class="grid md:grid-cols-3 gap-12">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8 hover-scale">
                        <svg class="w-12 h-12 text-[#FF2D20] mb-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg>
